...
export type TanTableFilterConfig =
  | SingleSelectFilterConfig
  | MultiSelectFilterConfig
  | RangeFilterConfig;

export type TanTableRangeValue = Partial<Record<RangeOperator, string>>;

export type TanTableFilterValue = string | string[] | TanTableRangeValue;

export type TanTableFilterValues = Record<string, TanTableFilterValue | undefined>;

interface TanTableFiltersProps {
  filters: TanTableFilterConfig[];
  values: TanTableFilterValues;
  onFilterChange: (filterId: string, value: TanTableFilterValue | undefined) => void;
  onClearAll?: () => void;
  isLoading?: boolean;
}
...
const isRangeValue = (value: TanTableFilterValue | undefined): value is TanTableRangeValue => {
  return !!value && !Array.isArray(value) && typeof value === "object";
};

const getFilterActiveCount = (
  filter: TanTableFilterConfig,
...
const RangeFilterControl = ({
  ...
  value: TanTableRangeValue;
  isLoading?: boolean;
  onFilterChange: (filterId: string, value: TanTableFilterValue | undefined) => void;
}) => {
  const [rangeValue, setRangeValue] = useState<TanTableRangeValue>(value);
...
const TanTableFilters = ({
  filters,
  values,
  onFilterChange,
  onClearAll,
  isLoading,
}: TanTableFiltersProps) => {
        ...
        for (const op of RANGE_OPERATORS) {
          ...
          if (val) {
            badges.push({
              ...
              onRemove: () => {
                const next: TanTableRangeValue = { ...filterValue, [op]: "" };
...
export default TanTableFilters;